...
def test_ready_preview_builds_authorization_required_without_granting_it():
    ...
    assert result["authorization_evidence_count"] == 2
    assert preview == snapshot


def test_non_fresh_after_preview_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(after_freshness={"status": "STALE", "execution_ready": False, "executed": False}))
...
def test_non_ready_after_preview_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(after_readiness={"review_status": "NEEDS_DATA_OR_REFRESH", "review_ready": False, "execution_ready": False, "executed": False}))
...
def test_unsafe_applied_evidence_is_blocked():
    ...
    result = build_freshness_evidence_application_authorization(_preview(applied_evidence=evidence))
    assert result["code"] == "APPLIED_EVIDENCE_UNSAFE"
    assert result["authorization_evidence"] == {}


def test_forged_preview_id_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(preview_id="evidence-application-preview:wrong"))
    assert result["code"] == "PREVIEW_ID_MISMATCH"


def test_request_id_must_bind_to_same_draft():
    result = build_freshness_evidence_application_authorization(_preview(request_id="refresh:d2"))
    assert result["code"] == "REQUEST_ID_MISMATCH"


def test_preview_that_already_allows_application_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(application_allowed=True))
    assert result["code"] == "APPLICATION_BOUNDARY_VIOLATION"
    assert result["application_started"] is False


def test_preview_execution_boundary_violation_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(execution_ready=True))
    assert result["code"] == "PREVIEW_SAFETY_BOUNDARY_VIOLATION"


def test_after_freshness_execution_boundary_violation_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(after_freshness={"status": "FRESH", "execution_ready": True, "executed": False}))
...
def test_after_readiness_execution_boundary_violation_is_blocked():
    result = build_freshness_evidence_application_authorization(_preview(after_readiness={"review_status": "READY_FOR_REVIEW", "review_ready": True, "execution_ready": True, "executed": False}))
