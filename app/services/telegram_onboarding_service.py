class TelegramOnboardingService:
    ...
    def start(self, user_id, main_keyboard):
        ...
        if account.get("connected") is not True:
            ...
            return {
                "error": False,
                "text": (
                    "Я помогу считать прибыль и находить риски по данным Ozon.\n\n"
                    "Шаг 1 из 2 — подключите кабинет Ozon. Отправьте одной строкой "
                    "CLIENT_ID и API_KEY через пробел. Ключ будет проверен только "
                    "READ-ONLY запросом, зашифрован и никогда не показан обратно."
                ),
                "onboarding": True,
                "required_step": "OZON_CREDENTIALS",
                "read_only_ozon": True,
                "executed_ozon": False,
            }
    ...
    def handle_callback(self, user_id, callback, main_keyboard):
        ...
        return {
            "error": False,
            "message": "Введите вашу налоговую ставку в процентах, например 6.",
            "onboarding": True,
            "required_step": "TAX_RATE",
        }
    ...
    def _tax_or_complete(self, user_key, main_keyboard, handled=False):
        ...
        result = {
            "error": False,
            "message": "Шаг 2 из 2 — выберите налоговый режим:",
            "keyboard": {"error": False, "type": "inline_keyboard", "buttons": [
                {"text": "УСН Доходы", "callback": "onboarding_tax:USN_INCOME"},
                {"text": "УСН Доходы − расходы", "callback": "onboarding_tax:USN_INCOME_MINUS_EXPENSES"},
                {"text": "Без налога", "callback": "onboarding_tax:NONE"},
            ]},
            "onboarding": True,
            "required_step": "TAX_CONFIGURATION",
        }
        if handled:
            result["handled"] = True
        return result
    ...
    def _complete(main_keyboard, handled=False):
        result = {
            "error": False,
            "text": "Настройка завершена. Выберите действие:",
            "message": "Настройка завершена. Выберите действие:",
