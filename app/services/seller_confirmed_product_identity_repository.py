...
class SellerConfirmedProductIdentityRepository:
    ...
    AUDIT_TABLE = "seller_confirmed_product_identity_mapping_audit"
    ...
    def revoke_mapping(
        ...
        """Remove one exact active projection while preserving its audit snapshot."""
        finance_key = self._text(finance_sku)
        current_key = self._text(current_sku)
        source_key = self._text(source)
        if not finance_key or not current_key or not source_key:
            return self._error("SELLER_PRODUCT_IDENTITY_REVOCATION_INPUT_INVALID")
        ...
        try:
            ...
            cursor.execute(
                "SELECT id, current_product_id, current_sku, current_offer_id, "
                f"source, confirmed_at FROM {self.TABLE} WHERE finance_sku = ?",
                (finance_key,),
            )
            row = cursor.fetchone()
            if row is None:
                conn.rollback()
                conn.close()
                return self._error("SELLER_PRODUCT_IDENTITY_MAPPING_MISSING")
            if self._text(row[2]) != current_key:
                conn.rollback()
                conn.close()
                return self._error("SELLER_PRODUCT_IDENTITY_REVOCATION_MISMATCH")
            cursor.execute(
                f"""
                INSERT INTO {self.AUDIT_TABLE} (
                    mapping_id, finance_sku, current_product_id, current_sku,
                    current_offer_id, mapping_source, confirmed_at,
                    event, event_source
                ) VALUES (?, ?, ?, ?, ?, ?, ?, 'REVOKED', ?)
                """,
                (
                    row[0], finance_key, row[1], current_key, row[3],
                    row[4], row[5], source_key,
                ),
            )
            cursor.execute(
                f"DELETE FROM {self.TABLE} WHERE id = ? AND finance_sku = ? "
                "AND current_sku = ?",
                (row[0], finance_key, current_key),
            )
            if cursor.rowcount != 1:
                raise RuntimeError("identity mapping changed during revocation")
            conn.commit()
            conn.close()
        except Exception:
            try:
        ...
        return {
            "error": False,
            "status": "SELLER_PRODUCT_IDENTITY_MAPPING_REVOKED",
            "finance_sku": finance_key,
            "current_sku": current_key,
    ...
    def _ensure_schema(cls, conn):
        ...
        cursor.execute(
            f"""
            CREATE TABLE IF NOT EXISTS {cls.AUDIT_TABLE} (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                mapping_id INTEGER NOT NULL,
                finance_sku TEXT NOT NULL,
                current_product_id TEXT NOT NULL,
                current_sku TEXT NOT NULL,
                current_offer_id TEXT,
        ...
        cursor.execute(
            f"CREATE INDEX IF NOT EXISTS idx_{cls.AUDIT_TABLE}_finance_sku "
            f"ON {cls.AUDIT_TABLE} (finance_sku)"
        )
        conn.commit()
