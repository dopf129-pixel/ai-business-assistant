        ...
        if isinstance(result, dict) and result.get("error") is True:
            ...
            if (
                ...
                if diagnostic.startswith("PERIOD_PROFIT_FINANCE_SKU_IDENTITY_"):
                    ...
                    message += (
                        "\nНажмите кнопку «Выбрать товар и исправить связь», "
                        "выберите текущий товар и повторите расчёт: бот покажет "
                        "кнопку подтверждения, если Ozon предоставляет "
                        "достаточные доказательства."
                    )
                    if skus:
                        message += (
                            "\nЕсли вы уже точно знаете текущий SKU из карточки "
                            "товара, отправьте боту фразу:\n"
                            "SKU " + skus[0]
                            + " и SKU ТЕКУЩИЙ_SKU — один товар"
                        )
                    message += (
