...
def build_freshness_evidence_application_permission_signal(eligibility, decision):
    ...
    return {
        "error": False,
        "permission_signal_id": "evidence-application-permission-signal:" + source["permission_eligibility_id"],
        ...
        "sku": source["sku"],
        "status": "APPLICATION_PERMISSION_GRANTED" if granted else "APPLICATION_PERMISSION_REJECTED",
        "decision": choice,
...
def _validate(source, choice):
    ...
    if any(not str(source.get(key) or "").strip() for key in required):
        return "APPLICATION_PERMISSION_SIGNAL_CONTEXT_REQUIRED"
    ...
    if source.get("status") != "APPLICATION_PERMISSION_REVIEW_REQUIRED":
        return "APPLICATION_PERMISSION_REVIEW_NOT_REQUIRED"
    if source.get("permission_eligible") is not True:
        return "APPLICATION_PERMISSION_NOT_ELIGIBLE"
    if source.get("permission_review_required") is not True:
        return "APPLICATION_PERMISSION_REVIEW_NOT_REQUIRED"
    if source.get("permission_granted") is not False:
        return "APPLICATION_PERMISSION_ALREADY_DECIDED"
    ...
    if source.get("persistent") is not False or source.get("source_freshness_proven") is not False:
        return "PERMISSION_ELIGIBILITY_BOUNDARY_VIOLATION"
    ...
    if any(source.get(field) is not False for field in safety_fields):
        return "PERMISSION_ELIGIBILITY_SAFETY_BOUNDARY_VIOLATION"
    ...
    if choice not in {"GRANT", "REJECT"}:
        return "APPLICATION_PERMISSION_DECISION_INVALID"
    return None
...
def _blocked(source, choice, code):
    return {
        ...
        "sku": source.get("sku"),
        "status": "APPLICATION_PERMISSION_SIGNAL_BLOCKED",
        "decision": choice,
        "permission_signal_ready": False,
