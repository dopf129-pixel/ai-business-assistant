...
class _Finance:
    ...
    def prefetch_daily_accruals(self, date_from, date_to):
        ...
        self._daily_accrual_cache[str(date_from)] = self._day_response()
        return {
            "error": False,
...
def test_scope_uses_prefetched_finance_posting_identity_without_fbo():
    summary = _Summary()
    finance = _Finance([
        {
    ...
    assert summary.received_products == [
        {
            "product_id": "product-1",
            "offer_id": "offer-1",
            "sku": "legacy-sku",
...
def test_scope_fails_closed_when_one_posting_has_multiple_catalog_skus():
    summary = _Summary()
    finance = _Finance([
