...
def _repos(tmp_path, monkeypatch):
    ...
    commit = ReturnCogsProfitApplicationCommitRepository()
    return recognition, authorization, commit


def _recognized(recognition, *, return_id="ret-1", posting="post-1", sku="sku-1", amount=120.5, confirmed_on="2026-09-01"):
    return recognition.record_recognition(
        return_id, posting, sku, "2026-08-31", "COGS_RECOVERY_RECOGNIZED",
        amount, "RUB", confirmed_on, source="TEST_ACCOUNTING",
    )


def _authorized(authorization, recognition_id, *, return_id="ret-1", posting="post-1", sku="sku-1", amount=120.5, confirmed_on="2026-09-02", state="PROFIT_APPLICATION_AUTHORIZED"):
    return authorization.record_application_state(
        recognition_id, return_id, posting, sku, "2026-08-31", state,
        amount if state != "PROFIT_APPLICATION_AUTHORIZATION_REVOKED" else None,
        "RUB" if state != "PROFIT_APPLICATION_AUTHORIZATION_REVOKED" else None,
        "EXCLUDED_FROM_ACCOUNT_NET_ACCRUAL" if state != "PROFIT_APPLICATION_AUTHORIZATION_REVOKED" else None,
        True if state != "PROFIT_APPLICATION_AUTHORIZATION_REVOKED" else None,
        confirmed_on,
...
def test_controlled_commit_derives_money_from_current_authorization(tmp_path, monkeypatch):
    ...
    assert result["application_commit_confirmed"] is True
...
def test_controlled_commit_rejects_stale_recognition_version(tmp_path, monkeypatch):
    ...
    auth = _authorized(authorization, rec["history_id"])
    newer = _recognized(recognition, amount=120.5, confirmed_on="2026-09-03")
...
def test_low_level_replay_with_changed_business_payload_is_conflict(tmp_path, monkeypatch):
    ...
    assert commit.list_commits()["count"] == 1
...
def test_ledger_integrity_reconciles_current_chain(tmp_path, monkeypatch):
    ...
    rec = _recognized(recognition)
    auth = _authorized(authorization, rec["history_id"])
    committed = commit.commit_current_authorization(
        rec["history_id"], auth["history_id"], "ret-1", "post-1", "sku-1", "2026-09-03"
    )
